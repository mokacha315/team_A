...
{
    // 公開フィールド
    // ★★★ 修正点: attackPowerはSwordHitコンポーネントのCurrentDamageを参照するように変更 ★★★
    public int attackPower
    {
        get
        {
            ...
            {
                return swordHitComponent.CurrentDamage; // バフ後の合計ダメージを返す
            }
            // 参照がない場合は、WeaponDataの基本値、または最低値1を返す
            return currentWeapon != null ? currentWeapon.attackPower : 1;
        }
    }
    ...
    void Attack()
    {
        ...
        // ★★★ デバッグログを追加: 攻撃開始時のダメージを確認 ★★★
        Debug.Log($"攻撃開始！ 現在の攻撃力: {attackPower}");
    ...
    public void EquipWeapon(WeaponData newWeapon)
    {
        ...
        // ★★★ 修正点: 生成した剣からSwordHitコンポーネントを取得し、キャッシュする ★★★
        swordHitComponent = sword.GetComponent<SwordHit>();
        if (swordHitComponent == null)
        {
            Debug.LogError("Sword prefab is missing the SwordHit component!");
        }

        // ★★★ 武器を生成し直した後、必ず初期位置を設定する！ ★★★
        SetInitialSwordPosition();

        Debug.Log("武器装備: " + newWeapon.weaponName + " / 攻撃力(基本): " + newWeapon.attackPower);
